...
public class OptimalWeight { /* Determining optimal weight */
    public static void main(String[] args) throws Exception {
        ...
        if (weight < optimalWeight) { // Compare the weight with optimum
            ...
            System.out.println("Вам необходимо поправится на " + result + " кГ"); // Output result
        }
        else
        if (weight > optimalWeight) {
            int result =  weight - optimalWeight;
            System.out.println("Вам необходимо похудеть на " + result + " кГ"); // Output result
        }
        else
        if (weight == optimalWeight) {
            System.out.println("Поздравляем! У Вас идеальный вес."); // Output result
        }
        }
}
